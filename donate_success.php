        ...
        for ($i = 0; $i < $count; $i++) {
            if (trim($_POST["ngo_name"][$i] != '') && trim($_POST["quantity_per_ngo"][$i] != '')) {
               ...
               //$get_Volunteer_email = "select email from volunteer INNER JOIN donation_quantity ON volunteer.registered_with_ngo=donation_quantity.ngo_Id where donation_Id='$donation_id' and pickup_location='$pickup_location'";
                ...
                $query_ngo_mail=mysqli_query($conn,"select * FROM ngo where ngoId='$ngo_name'") or die(mysqli_error($conn));
                $row_ngo_email= mysqli_fetch_array($query_ngo_mail);
                $ngo_email_val=$row_ngo_email['email'];
                $ngo_name_mail=$row_ngo_email['ngo_name'];
                echo $ngo_email_val;

            include 'mail2.php';
            $mail->addAddress($ngo_email_val);
            $mail->Subject = 'New Donation';
            $mail->Body    = 'Dear ' .$ngo_name_mail.', there is a new donation .Kindly Check your dashboard .The notification has been sent to your volunteer for pickup';
...
if (mysqli_query($conn, $query_donation)) {
    try {
        ...
        $get_Volunteer_email = "select * from volunteer INNER JOIN donation_quantity ON volunteer.registered_with_ngo=donation_quantity.ngo_Id where donation_Id='$donation_id' and volunteer.pickup_location='$pickup_location' and volunteer.flag='1'";
